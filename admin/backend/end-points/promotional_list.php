...
if ($fetch_all_promotion): ?>
    <?php foreach ($fetch_all_promotion as $promotion):
        ...
        <tr>
            ...
            <td class="p-2">
                <button class="bg-blue-500 text-white py-1 px-3 togglerUpdateUser" data-user_id=<?=$promotion['promo_id']?>>Update</button>
                <button class="bg-red-500 text-white py-1 px-3 " data-user_id=<?=$promotion['promo_id']?>>Remove</button>
            </td>
        </tr>
    <?php endforeach; ?>
<?php else: ?>

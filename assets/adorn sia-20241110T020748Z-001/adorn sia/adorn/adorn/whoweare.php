<?php include "server.php"; ?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Adorn.</title>
    <link rel="stylesheet" href="Adornwho.css"> 
    <link rel="stylesheet" href="Adornstyle.css">
    <link rel="icon" type="image/x-icon" href="images/logo1.png">
    
    <!----===== Boxicons CSS ===== -->
    <link href='https://unpkg.com/boxicons@2.1.1/css/boxicons.min.css' rel='stylesheet'>
</head>

<body>
    <div class="nav">
        <input type="checkbox" id="sidebar-active">
        <label for="sidebar-active" class="open-sidebar-button">
            <svg xmlns="http://www.w3.org/2000/svg" height="32" viewBox="0 -960 960 960" width="32">
                <path d="M120-240v-80h720v80H120Zm0-200v-80h720v80H120Zm0-200v-80h720v80H120Z" />
            </svg>
        </label>
        <label id="overlay" for="sidebar-active"></label>
        <div class="links-container">
            <label for="sidebar-active" class="close-sidebar-button">
                <svg xmlns="http://www.w3.org/2000/svg" height="32" viewBox="0 -960 960 960" width="32">
                    <path
                        d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z" />
                </svg>
            </label>

            <a href="index.html" class="nav-logo-link">
                <img src="images/nav_logo2.png" alt="Navbar Logo" class="nav-logo">
            </a>

            <a href="index.php">Home</a>
            <a href="products.php">Products</a>
            <a href="whoweare.php" class="active">Who we are</a>
            <a href="contactus.php">Contact us</a>
            <div class="icons-container">
                <a href="#" class="nav-icon search">
                    <img src="data:image/png;base64,"/>
                </a>
                <a href="#" class="nav-icon cart">
                    <img src="data:image/png;base64,"/>
                </a>       
                <?php if (!isset($_SESSION['username'])) {
                    echo "<a href='login.php' class='login-btn'>Log In</a>";
                } else {
                    echo "<a href='profile.php' class='login-btn'>"; echo $_SESSION['username']; echo"</a>";
                }
                 ?> 

            </div>            
        </div>
    </div>

    <!-- Best, Adorn Section-->
    <section class="who-we-are-header" id="about-us">
        <h1>ADORN; Style that speaks</h1>
        <div class="who-we-are-container">

            <div class="poster-image">
                <img src="images/poster1.png" alt="Company Poster">
            </div>

            <div class="description">
                <p>
                Adorn focuses on developing a cutting-edge e-commerce platform specifically for streetwear enthusiasts. The website will cater to a
                 diverse range of styles, from bold graphic tees to trendy hoodies and accessories. By analyzing market trends 
                 and consumer preferences, the project aims to create a user-friendly and reliable online 
                store for streetwear fashion, ensuring a seamless shopping experience and meeting the growing demand for convenient online apparel sales.
                </p>
            </div>
        </div>
    </section>

    <!-- Vision and Mission -->
    <section class="vision-mission-section">
        <div class="vision-mission-container">

            <div class="vision" id="our-vision">
                <h2>Our <i>Vision</i></h2>
                <p>
                To be the leading online destination for streetwear enthusiasts, delivering an exceptional shopping experience 
                with a curated selection of unique and stylish outfits.We aim to set the standard for convenience, security,
                 and customer satisfaction in the e-commerce fashion industry.
                </p>
            </div>

            <div class="mission" id="our-mission">
                <h2>Our <i>Mission</i></h2>
                <p>
                Our mission is to provide a seamless and personalized online shopping experience for streetwear enthusiasts.
                 We offer a diverse range of trendy outfits that reflect individual style and culture. Committed to secure 
                 transactions and reliable service, we strive to meet the evolving needs of our customers while promoting 
                 self-expression through fashion.
                </p>
            </div>
        </div>
    </section>

    <!-- Footer Section -->
    <footer>
        <div class="footer-container">

            <div class="footer-column">
                <img src="images/footer_logo2.png" alt="Logo" class="footer-logo">
                <p>© 2024 Adorn, All rights reserved.</p>
            </div>

            <div class="footer-column">
                <h3>Company</h3>
                <a href="whoweare.php#about-us">About Us</a>
                <a href="whoweare.php#our-vision">Our Vision</a>
                <a href="#our-mission">Our Mission</a>
            </div>

            <div class="footer-column">
                <h3>Resources</h3>
                <a href="#faq">FAQ</a>
                <a href="#shipping-info">Shipping Info</a>
                <a href="#returns">Returns</a>
            </div>

            <div class="footer-column">
                <h3>Connect</h3>
                <a href="#instagram">Instagram</a>
                <a href="#facebook">Facebook</a>
                <a href="mailto:email@example.com">Email</a>
            </div>
        </div>
    </footer>





    <script src="home.js"></script>
</body>

</html>
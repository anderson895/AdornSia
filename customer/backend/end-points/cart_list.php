...
<div class="flex flex-col lg:flex-row items-center border-t border-gray-200 pt-6 relative lg:space-x-6">
    ...
    <div class="flex-grow text-center lg:text-left">
        ...
        <p class="text-xs md:text-sm text-gray-600"><?= substr($cart['prod_description'], 0, 50) ?></p>
        <p class="text-xs md:text-sm text-gray-600">Size: <?=$cart['cart_prod_size']?></p>
        <p class="text-xs md:text-sm text-gray-600">Original price: <?=$cart['prod_currprice']?></p>
        <p class="text-xs md:text-sm text-gray-600">Available stocks: <?=$cart['product_stocks']?></p>
        ...
        <div class="flex items-center justify-center lg:justify-start space-x-2 mt-2">
            ...
            <input readonly type="number" value="<?=$cart['cart_Qty']?>" 
                   class="w-16 h-8 text-center border border-gray-300 rounded-md px-2 py-1 text-sm" 
                   placeholder="Qty: 1" min="1" />
    ...
    <div class="text-right mt-4 lg:mt-0">
        <?php if ($cart['prod_promo_id']): ?>
            <p class="text-red-600 font-semibold text-sm md:text-base">Php <?=number_format(($cart['prod_currprice'] - ($cart['prod_currprice'] - $discounted_price)) * $cart['cart_Qty'], 2)?></p>
        ...
        <?php else: ?>
            <p class="text-red-600 font-semibold text-sm md:text-base itemPrice">Php <?=number_format($cart['prod_currprice'] * $cart['cart_Qty'], 2)?></p>
        <?php endif; ?>
    </div>
</div>
